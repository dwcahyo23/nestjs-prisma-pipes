...
describe('AggregatePipe', () => {
    ...
    describe('Basic Aggregation', () => {
        ...
        it('should parse all aggregate functions', () => {
            const result = pipe.transform('total: sum(), average: avg(), minimum: min(), maximum: max(), recordCount: count()');
    ...
    describe('GroupBy', () => {
        ...
        it('should parse nested groupBy fields and use raw query', () => {
            const result = pipe.transform('qty: sum(), groupBy: (marketingMasterCategory.category)');
        ...
        it('should parse multiple nested paths and use raw query', () => {
            const result = pipe.transform('qty: sum(), groupBy: (marketingMasterCategory.category, warehouse.region)');
    ...
    describe('Chart Configuration', () => {
        ...
        it('should parse chart with groupField', () => {
            const result = pipe.transform('qty: sum(), groupBy: (category), chart: bar(category)');
        ...
        it('should parse chart with nested path and use raw query', () => {
            const result = pipe.transform('qty: sum(), groupBy: (marketingMasterCategory.category), chart: bar(marketingMasterCategory.category)');
            expect(result?.chartConfig?.type).toBe('bar');
            expect(result?.chartConfig?.groupField).toBe('marketingMasterCategory.category');
            expect(result?.useRawQuery).toBe(true);
        });
        it('should parse chart with options', () => {
            const stacked = pipe.transform('qty: sum(), groupBy: (category), chart: bar(category, stacked)');
            expect(stacked?.chartConfig?.stacked).toBe(true);
            const horizontal = pipe.transform('qty: sum(), groupBy: (category), chart: bar(category, horizontal)');
    ...
    describe('Time Series', () => {
        ...
        it('should parse all time intervals', () => {
            expect(pipe.transform('qty: sum(), chart: line(createdAt, day)')?.chartConfig?.interval).toBe('day');
            expect(pipe.transform('qty: sum(), chart: line(createdAt, month)')?.chartConfig?.interval).toBe('month');
            expect(pipe.transform('qty: sum(), chart: line(createdAt, year)')?.chartConfig?.interval).toBe('year');
        });
    });
    describe('Grouped Time Series', () => {
        it('should parse grouped time series', () => {
            const result = pipe.transform('qty: sum(), groupBy: (category, createdAt), chart: line(createdAt, month)');
        ...
        it('should handle multiple aggregates with grouped time series', () => {
            const result = pipe.transform('qty: sum(), recQty: sum(), groupBy: (category, createdAt), chart: line(createdAt, month)');
            expect(result?.aggregates).toHaveLength(2);
            expect(result?.groupBy).toEqual(['category', 'createdAt']);
            expect(result?.useRawQuery).toBe(false);
        });
        it('should parse nested path with time series and use raw query', () => {
            const result = pipe.transform('qty: sum(), groupBy: (marketingMasterCategory.category, createdAt), chart: line(createdAt, month)');
    ...
    describe('Raw Query Generation', () => {
        ...
        it('should generate raw query with multiple aggregates', () => {
            const result = pipe.transform('qty: sum(), recQty: avg(), groupBy: (marketingMasterCategory.category)');
            if (result?.rawQueryBuilder) {
                const { query } = result.rawQueryBuilder('Product');
                expect(query).toContain('SUM(main.qty)');
    ...
    describe('Error Handling', () => {
        it('should throw for invalid syntax', () => {
            expect(() => pipe.transform('x = ???')).toThrow(common_1.BadRequestException);
        });
        it('should throw when no aggregate is provided', () => {
            expect(() => pipe.transform('chart: bar')).toThrow(common_1.BadRequestException);
        });
        it('should return undefined for empty string', () => {
            expect(pipe.transform('')).toBeUndefined();
        });
        it('should throw for invalid aggregate function', () => {
            expect(() => pipe.transform('qty: invalid()')).toThrow(common_1.BadRequestException);
        });
        it('should throw for groupBy without parentheses', () => {
            expect(() => pipe.transform('qty: sum(), groupBy: category')).toThrow(common_1.BadRequestException);
        });
    });
    describe('toChartSeries - Non-Grouped', () => {
        it('should return correct series for single aggregate', () => {
            ...
            const chart = aggregate_pipe_1.default.toChartSeries(prismaResult, config);
        ...
        it('should handle empty data', () => {
            const config = pipe.transform('qty: sum()');
            const chart = aggregate_pipe_1.default.toChartSeries([], config);
            expect(chart.categories).toEqual(['Total']);
    ...
    describe('toChartSeries - Grouped', () => {
        it('should group by category and produce chart series', () => {
            ...
            const chart = aggregate_pipe_1.default.toChartSeries(data, config);
        ...
        it('should handle multiple aggregates with grouping', () => {
            const config = pipe.transform('qty: sum(), recQty: sum(), groupBy: (category)');
            const data = [
        ...
        it('should handle multiple groupBy fields', () => {
            const config = pipe.transform('qty: sum(), groupBy: (category, region)');
            const data = [
    ...
    describe('toChartSeries - Time Series', () => {
        it('should generate monthly time series', () => {
            ...
            expect(chart.series[0].data[0]).toBe(10);
            expect(chart.series[0].data[1]).toBe(20);
            expect(chart.series[0].data[2]).toBe(15);
        });
        it('should handle multiple aggregates in time series', () => {
            const config = pipe.transform('qty: sum(), recQty: sum(), chart: line(createdAt, month)');
            const data = [
        ...
        it('should fill missing months with 0', () => {
            ...
            expect(chart.series[0].data[2]).toBe(0);
        });
    });
    describe('toChartSeries - Grouped Time Series', () => {
        it('should build separate series per category', () => {
            const config = pipe.transform('qty: sum(), groupBy: (category, createdAt), chart: line(createdAt, month)');
            const data = [
            ...
            expect(chart.series[0].data[1]).toBe(7);
            expect(chart.series[1].data[0]).toBe(10);
            expect(chart.series[1].data[1]).toBe(12);
        });
        it('should handle multiple aggregates with grouped time series', () => {
            const config = pipe.transform('qty: sum(), recQty: sum(), groupBy: (category, createdAt), chart: line(createdAt, month)');
            const data = [
                {
                    category: 'COM',
    ...
    describe('Prisma Query Generation', () => {
        ...
        it('should generate correct query for time series', () => {
            const config = pipe.transform('revenue: sum(), chart: line(createdAt, month)');
            expect(config?.prismaQuery).toEqual({
                by: ['createdAt'],
                _sum: { revenue: true },
            });
