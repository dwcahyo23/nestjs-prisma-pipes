...
function testEncoding() {
	...
	for (const query of TEST_QUERIES) {
		try {
			const encoded = encodePipeQuery(query, TEST_SECRET);
			console.log(`✅ "${query}"`);
...
function testRoundTrip() {
	...
	for (const query of TEST_QUERIES) {
		try {
			...
			// Decode
			const decoded = decodePipeQuery(encoded);
...
function testExpiredQuery() {
	...
	try {
		// Create an expired query manually
		...
		// Try to decode
		decodePipeQuery(expiredQuery);

		console.log('❌ Expired query should have been rejected!');
	} catch (error) {
		console.log('✅ Expired query correctly rejected:',
...
function testInvalidSignature() {
	...
	try {
		const encoded = encodePipeQuery('valid-query', TEST_SECRET);

		// Tamper with the encoded string
		const tampered = encoded.slice(0, -5) + 'XXXXX';

		// Try to decode tampered query
		decodePipeQuery(tampered);

		console.log('❌ Tampered query should have been rejected!');
	} catch (error) {
		console.log('✅ Tampered query correctly rejected:',
			error instanceof Error ? error.message : error);
	}
}
...
function generateTestUrl() {
	...
	for (const [key, value] of Object.entries(params)) {
		const encoded = encodePipeQuery(value, TEST_SECRET);
