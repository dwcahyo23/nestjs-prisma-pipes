...
describe('WherePipe', () => {
	...
	describe('Date range in same column', () => {
		it('should parse date range (gte & lte) in the same column', () => {
			const dateRange =
				'createdAt: gte date(2024-01-01T00:00:00Z), createdAt: lte date(2024-12-31T23:59:59Z)';
			...
			const createdAt = result!.createdAt as any;
			expect(createdAt.gte).toBe('2024-01-01T00:00:00.000Z');
			expect(createdAt.lte).toBe('2024-12-31T23:59:59.000Z');

			// Validate date range: gte <= lte
			expect(new Date(createdAt.gte).getTime()).toBeLessThanOrEqual(
				new Date(createdAt.lte).getTime()
			);
		});

		it('should parse date range (gt & lt) in the same column', () => {
			const dateRange =
				'publishedAt: gt date(2024-06-01T00:00:00Z), publishedAt: lt date(2024-06-30T23:59:59Z)';
	...
	describe('Nested relation filters', () => {
		...
		it('should parse deeply nested ".some" relation filter', () => {
			const result = pipe.transform(
				'company.is.departments.some.employees.every.name: contains string(John)'
			);
			expect(result).toEqual({
				company: {
					is: {
	...
	describe('Complex combinations', () => {
		...
		it('should parse multi-nested object correctly', () => {
			const result = pipe.transform(
				'hseAparRevision.unit.location.city: string(Jakarta), ' +
